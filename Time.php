...
class Time {
     ...
     * @return string
     */
    public function countSeniority( $time ,$precision=false) {
        $diff = abs($this->parse($time) - $this->date);
        static $chunks = array(array(31536000,'年'),array(2592000,'月'));
        $count =0;
        $since = '';
        for($i=0;$i<count($chunks);$i++) {
            if($diff>=$chunks[$i][0]) {
        ...
        if(!strstr($since, '月')) $since .= '0月';
        if(!strstr($since, '年')) $since = '0年' . $since;
        return $since;
    }
